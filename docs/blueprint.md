...
- **デフォルトゴミ種別の完全性**: 新規ユーザー、または場所追加時のデフォルト設定として、以下の6種別を必ず初期セットとして保持すること（可燃ごみ、不燃ごみ、プラスチック、資源ごみ、ペットボトル、古紙）。
- **個別回収管理**:
  - 「粗大ごみ」「不用品回収」の2つのサブタブ。
...
- **トグルスイッチ**:
    - 独自のCSSスタイルを適用し、標準のチェックボックスをiOSスタイルのモダンなトグルスイッチ（背景アニメーション付き）として表現する。

### 4.4 データ永続化・安全性（★重要：省略禁止）
- **初期化の非破壊原則**:
    - `loadAppData` 処理において、ローカルストレージに既存データ（`locations` や `trashRules`）が存在する場合、いかなる理由があってもコード内のデフォルト値で上書き（破壊）してはならない。
- **構造の欠損補完**:
    - データ構造が一部欠損している場合は、既存のユーザーデータを維持したまま、不足しているプロパティのみをマージして補完すること。
- **フルセットの維持**:
    - ゴミ種別のデフォルト値をコード内で記述する場合、必ず「可燃ごみ・不燃ごみ・プラスチック・資源ごみ・ペットボトル・古紙」の6種フルセットを記述すること。開発効率のために一部を省略（「可燃のみ」など）してはならない。
```
